...
public class Ex01_BookTest {
	...
	public void equalsTest() {
		Book b1 = new Book("Nam39rou7d", "최고의 딴따라", 20000);
		// assertEquals(b1.author, "햄스터");
		assertEquals(b1.author, "Nam39rou7d");
	...
	public void sameTest() {
		Book b1 = new Book("Nam39rou7d", "최고의 딴따라", 20000);
		Book b2 = new Book("톨스토이","죄와 벌",15000);
		Book b3 = new Book("톨스토이","죄와 벌",15000);
		// assertSame(b1,b2); // false
		// assertSame(b3,b2); // false
		b1 = b3;
	...
	public void nullTest() {
		Book b1; 
		// => 인스턴스를 정의만하고 생성 하지 않은상태
		//    지역변수는 초기화 하지 않으면 오류, 사용시 오류발생
		Book b2 = new Book("톨스토이","죄와 벌",15000); // 인스턴스 생성
		b2 = null; // 인스턴스 해제(삭제)
		assertNotNull(b2);
	}
	...
	public void arrayEqualsTest() {
		...
		String[] a3 = {"나", "가", "다"};
		String[] a4 = {"가", "나", "라"};
		// 5.1) 두배열의 순서, 값 모두 동일, 주소값만 다름 (a1, a2)
		// => true : 순서(index)와 값을 비교 (주소비교 아님)
		// assertArrayEquals(a1, a2); // true
		
		// 5.2) 두배열의 순서가 다르고, 값 모두 동일 (a1, a3)
		...
		// => 각 배열의 Data 는 3개씩
		Book b1 = new Book("Nam39rou7d", "최고의 딴따라", 20000);
		Book b2 = new Book("NAMEGROUND", "최고의 딴따라", 17000);
		Book b3 = new Book("NameGround", "최고의 딴따라", 25000);
		Book [] ba1 = {b1, b2, b3};
		Book [] ba2 = {b1, b2, new Book("김진휘", "딴따라", 15000)};
		Book [] ba3 = {b1, b2, b3};
