...
//=> 추상메서드가 하나라도 정의되면 그 클래스는 반드시 추상클래스로 정의 해야 함 = abstract를 붙여야함
...
class Cat extends Animal {
	...
	@Override
	void sound() {
		System.out.println("야옹야옹 웁니다.");
	}
	@Override
	void special() {
		System.out.println("special : 고양이 eyeColor 는 멋집니다.");
	}
	// myMethod
	void eyeColor() {
		System.out.println("고양이 eyeColor 는 멋집니다.");
	}
}
...
class Dog extends Animal {
	...
	@Override
	void special() {
		System.out.println("special : 강아지는 빠르게 달립니다.");
...
class Eagle extends Animal {
	...
	@Override
	void sound() {
		System.out.println("수리 수리 웁니다.");
	}
	@Override
	void special() {
		System.out.println("훨훨 높이 날아다닙니다.");
	}
}
...
class Bear {
	...
	void sound() {
		System.out.println("어흥 어흥 웁니다.");
	}
	void special() {
		System.out.println("덩치는 크지만 나무를 잘탑니다.");
	}
...
public class Ex01_AbsAnimal {
	...
	public static void main(String[] args) {
		...
		// 2) Cat 생성
		// => 후손생성시 추상클래스인 조상의 생성자 호출함
		// 	  그렇더라도 추상클래스로 직접 인스턴스 생성은 불가
		...
		//    접근 가능한 멤버의 범위는 Type에 정의된 만큼 사용가능
		//    즉, Cat 에만 정의된 메서드는 Animal Type 에서는 접근 불가
		...
		d1.breath();
		d1.sound();
		// d1.speed(); -> Error : Method speed() undefined for the Type Animal
		System.out.println("");
		...
		if(c2 instanceof Animal) {
			System.out.println("c2 는 Animal 입니다.");
		}else {
		...
		//    interface 의 경우에는 런타임시에 오류발생 하므로 instanceof 연산자를 이용한 확인 필요함
...
//		}
	...
	public static void animalUse(Object animal) {
		System.out.println("Your Address => " + animal.toString());
	}
}
