...
public class Ex03_Method {
	...
	int price = 5000; // 전역 변수(멤버, Global 변수)

	// 1) 매개변수, return 값 없음.

	public void juiceCafe1 () { // 메서드 헤더,  
		System.out.println("----- 1 -----");
		System.out.println("무슨 음료를 원하시나요?"); // {...실행문...} 메서드의 바디
	}

	// 메서드명
	// => 일반적인 식별자 규칙을 준수
	// => 소문자로 시작
	// => 동일한 이름의 메서드명 허용 (매개변수의 개수, 타입이 다른경우)
	...
	public void juiceCafe2 (String s, int n) {
		...
		System.out.printf("%s 음료 , %d 잔 \n", s, n);

		// ------------------------------------------

		// 매개변수의 전달 Test (CallByValue)
	...
	// 3) 매개변수없고, return 값 있음

	public String juiceCafe3 () {
		System.out.println("----- 3 -----");
		return "음료 1잔은 : " + price + " 원 입니다.";
	}

	// ----------------------------------------------------- 3)

	// 4) 매개변수있고, return 값 있음

	public int juiceCafe4(String s, int n) {
		System.out.println("----- 4 -----");
		System.out.printf("=> %s 음료, %d 잔 총액 \n", s, n);
		return price * n;
	}
	...
	public static void main(String[] args) {
		...
		// juiceCafe1(); , price = 9000; -> 인스턴스 없이 호출안됨. == Error
		// 2,4은 매개변수가 있어야 호출이 가능

		Ex03_Method ex03 = new Ex03_Method();
		ex03.juiceCafe1();
		ex03.juiceCafe2("수박" , 5);



		// ex03.juiceCafe2(5, "수박"); 호출시 반드시 인자가(순서까지) 일치해야함
		System.out.println("juiceCafe3 => " + ex03.juiceCafe3());
		System.out.println("juiceCafe4 => " + ex03.juiceCafe4("오렌지" , 4));
		...
		// 매개변수 전달 Test1 기본자료형(CallByValue)
		String kind = "딸기";
		ex03.juiceCafe2(kind , 5);
		System.out.println("Main 에서 찍은 kind => " + kind);
		
		

		// 매개변수 전달 Test2 참조자료형(CallByReference)
		Car c1 = new Car();
		c1.color = "Blue";
		c1.speed = 1000;
		c1.mileage = 1000;
		...
		System.out.println("Main 바로 생 return 값 => " + ex03.carTest(new Car(), 999)); // Pink
