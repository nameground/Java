 ...
 * 
 * -- 호출규칙
 * => static 메서드 : static 만 인스턴스없이 호출 가능.
 * => instance 메서드 : static, instance(non_static) 모두 인스턴스 없이 호출 가능
 * 
 * 
 */
...
public class Ex05_Static {
	...
	public static int add(int i , int j, Ex05_Static ex00) {
		// result = i+j; -> 인스턴스 없이는 접근불가
		
		total += (i + j); // -> static 메서드에서는 static 은 인스턴스 없이 접근 가능
		// static 메서드에서 인스턴스 추가
		// =>  static 메서드에서는 인스턴스는 인스턴스 있어 접근 가능
	...
	// => instance
	// => static, instance(non_static) 모두 인스턴스 없이 호출 가능
	...
	public static void main(String[] args) {
		...
		System.out.println("min => " + min(10, 3, ex05));
		...
		// 1.3) staticAll call
		System.out.println("3) staticAll call");
		staticAll(30, 3, ex05);
		
		// 1.4) instanceAll call
		System.out.println("4) instanceAll call");
		...
		System.out.println("min2 => " + min(10, 3, ex055));
		...
		// 2.3) staticAll call
		System.out.println(" 2.3) staticAll call");
		staticAll(30, 3, ex055);
		
		// 2.4) instanceAll call
		System.out.println(" 2.4) instanceAll call");
		ex05.instanceAll(40, 4, ex055);
		
		// System.out.println(" Test1 => " + ex05.total); // 경고, 바림직한 접근은 아님
		System.out.println(" Test2 => " + Ex05_Static.total); // 클래스 종속
